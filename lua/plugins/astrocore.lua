...
return {
  ...
  opts = function(_, opts)
    -- Merge with default opts using extend_tbl to preserve defaults
        ...
        n = {
          ...
          ["<leader>fo"] = {
            function()
              ...
              local root = root_pattern(vim.api.nvim_buf_get_name(0)) or vim.fn.getcwd()
              require("snacks.picker").recent { cwd = root }
            end,
            desc = "Recent files (project)",
          },
          -- Neo-tree explorer (floating, git status first)
