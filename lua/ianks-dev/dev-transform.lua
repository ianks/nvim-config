...
M.transform = function(cmd)
  ...
  if not M.has_dev_yml() then
    return cmd
  end
  
  -- Only transform Ruby test commands
  -- Check for common Ruby test patterns
  if cmd:match('rake test') or 
     cmd:match('rails test') or
     cmd:match('rspec') or
     cmd:match('minitest') or
     cmd:match('cucumber') or
     cmd:match('ruby %-Itest') or
     cmd:match('ruby %-Ispec') or
     cmd:match('bundle exec rake test') or
     cmd:match('bundle exec rails test') or
     cmd:match('bundle exec rspec') or
     cmd:match('bin/rake test') or
     cmd:match('bin/rails test') or
     cmd:match('bin/rspec') or
     cmd:match('zeus') or
     cmd:match('spring') then
    ...
    if not test_file then
      test_file = cmd:match('([%w%p/]+_spec%.rb)')
    end
    if not test_file then
      test_file = cmd:match('(test/[%w%p/]+%.rb)')
    end
    if not test_file then
      test_file = cmd:match('(spec/[%w%p/]+%.rb)')
    end
