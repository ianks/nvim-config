...
local function tmux_cmd(cmd)
  ...
  if vim.v.shell_error ~= 0 then
    error("Tmux command failed: " .. result)
  end
...
local function send_keys(keys)
  tmux_cmd("send-keys -t " .. test_pane_id .. " " .. keys)
end

-- Send text instantly with literal flag (no typing animation)
local function send_text(text)
  tmux_cmd("send-keys -t " .. test_pane_id .. " -l " .. vim.fn.shellescape(text))
end

M.get_or_create_test_pane = function()
  if not in_tmux() then
    error("ianks-dev: Not running inside tmux session")
  end
  
  -- Check if pane exists and is still valid
  if test_pane_id and has_runner() then
    return test_pane_id
  end
  ...
  local result = tmux_cmd("split-window -v -p 30 -c '#{pane_current_path}' -e NVIM_TEST_PANE=1 -P -F '#{pane_id}' '$SHELL -l'")
...
M.send_to_pane = function(pane_id, cmd)
  ...
  local result = vim.fn.system({"tmux", "send-keys", "-t", test_pane_id, 
                                 "C-c", "C-m", "clear", "C-m"})
  ...
  if vim.v.shell_error ~= 0 then
    ...
    M.get_or_create_test_pane()
    vim.fn.system({"tmux", "send-keys", "-t", test_pane_id, 
                   "C-c", "C-m", "clear", "C-m"})
    vim.fn.system({"tmux", "send-keys", "-t", test_pane_id, "-l", cmd})
    vim.fn.system({"tmux", "send-keys", "-t", test_pane_id, "C-m"})
  end
...
M.cleanup = function()
  if test_pane_id and has_runner() then
    tmux_cmd("kill-pane -t " .. test_pane_id)
  end
  test_pane_id = nil
end
